...
addpath(genpath(RDHEI_ROOT));   % 如果你确认不会污染内置函数，保留；否则换成更精确的子目录 addpath
...
%% ========= [F] Huffman encode labels (canonical Huffman) =========
...
%% ========= [F] Canonical Huffman encode labels =========
% symbols are 0/1 uint8
symbols = labels_u8(:);

% 1) compute canonical Huffman code lengths from data
%    (function name in your repo: huffman_codelens.m)
lens = huffman_codelens(symbols);

% 2) build canonical codebook (function: canonical_codebook.m)
cb = canonical_codebook(lens);

% 3) encode to bitstream (function: huff_encode_symbols.m)
% ===== Label bits (NO Huffman; binary labels are already 1 bit/symbol) =====
lab_bits = uint8(labels_u8(:).');      % 1x8192, values 0/1
...
% ---------- Huffman self-check (no RDHEI) ----------
dm0 = huff_build_decodemaps(cb);
lab_bits0 = lab_bits;  % encoded from labels_u8

labels_check = huff_decode_bits(lab_bits0, dm0, uint32(numel(labels_u8)));
...
% NOTE: 你工程里 pack_header_bits/parse_header_bits 的签名可能不同
% 下面写法是“优先带 Huffman dict”，如果失败就回退到只 pack H
...
% Pack header
try
    hb = pack_header_bits(H, HUFF);   % <-- 如果你的 header 需要携带 Huffman 信息，最好走这条
    header_has_huff = true;
catch
    hb = pack_header_bits(H);         % <-- 你的旧版本可能只有 H
    header_has_huff = false;
    warning('pack_header_bits(H,HUFF) failed. Using pack_header_bits(H). If header must carry Huffman dict, fix pack_header_bits/parse_header_bits signature.');
end
...
% Huffman decode labels and compare
%% ========= Huffman decode labels =========
% build decode maps from the SAME canonical codebook
